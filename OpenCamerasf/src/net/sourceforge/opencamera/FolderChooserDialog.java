...
public class FolderChooserDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		File new_folder = CameraActivityNew.getImageFolder(folder_name);
		if( MyDebug.LOG )
    ...
    private boolean useFolder() {
		...
		if( current_folder == null )
			return false;
		if( canWrite() ) {
        	File base_folder = CameraActivityNew.getBaseFolder();
			...
			editor.putString(CameraActivityNew.getSaveLocationPreferenceKey(), new_save_location);
			editor.apply();
			return true;
		}
		else {
